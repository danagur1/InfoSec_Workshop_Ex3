...
import sys
import os
sys.path.append(os.path.abspath('../conn_functions'))
...
def handle_client(client_socket, address, used_ports):
    ...
    my_port = get_proxy_port_http(src_ip, dst_ip, src_port, dst_port)
    if my_port=="any": #my port have not been set yet
        my_port = generate_unique_port(used_ports)
        set_proxy_port_http(src_ip, dst_ip, src_port, dst_port, my_port)
